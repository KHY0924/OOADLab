    ...
    public List<Material> getMaterialsBySubmissionId(String submissionId) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    materials.add(new Material(rs.getString("material_id"), rs.getString("submission_id"), rs.getString("file_name"), rs.getString("file_type"), rs.getTimestamp("upload_date"), rs.getString("file_path")));
                }
            }
