...
public class PresentationBoardDAO {
    ...
    public PresentationBoard getBoardById(int boardId) {
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                return new PresentationBoard(rs.getInt("board_id"), rs.getString("board_name"), rs.getString("location"), rs.getInt("max_presentations"), rs.getInt("current_presentations"), rs.getString("session_id"), rs.getString("presentation_type"));
            }
        } catch (SQLException e) {
    ...
    public List<PresentationBoard> getAllBoards() {
        ...
        try (Connection conn = DatabaseConnection.getConnection(); Statement stmt = conn.createStatement()) {
            ...
            while (rs.next()) {
                boards.add(new PresentationBoard(rs.getInt("board_id"), rs.getString("board_name"), rs.getString("location"), rs.getInt("max_presentations"), rs.getInt("current_presentations"), rs.getString("session_id"), rs.getString("presentation_type")));
            }
        } catch (SQLException e) {
