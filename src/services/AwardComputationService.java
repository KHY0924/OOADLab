...
public class AwardComputationService {
    ...
    public Award computeBestOralPresentation(List<Submission> oralSubmissions, List<Evaluation> evaluations) {
    ...
    public Award computeBestPosterPresentation(List<Submission> posterSubmissions, List<Evaluation> evaluations) {
    ...
    public Award computePeoplesChoice(List<Submission> allSubmissions, Map<String, Integer> peopleVotes) {
        if (allSubmissions == null || allSubmissions.isEmpty() || peopleVotes == null || peopleVotes.isEmpty()) {
            return null;
        }
    ...
    private Map<String, Double> calculateAverageScores(List<Submission> submissions, List<Evaluation> evaluations) {
        ...
        for (Evaluation evaluation : evaluations) {
            submissionEvaluations.computeIfPresent(evaluation.getSubmissionId(), (key, scores) -> {
    ...
    private Award createAward(String awardType, Submission submission) {
        ...
        return new Award(awardId, awardType, submission.getStudentId(), submission.getStudentName(), submission.getTitle(), submission.getSeminarId());
    }
    ...
    public boolean validateAwardComputation(List<Submission> oralSubmissions, List<Submission> posterSubmissions, List<Evaluation> evaluations) {
